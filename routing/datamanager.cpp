...
DataManager::requestRoadsFromNode(unsigned long long idNode)
{
     ...
     //execute
     if(!query.exec())
       qWarning() << "ERROR Finding roads in requestRoadFromNode: " << query.lastError().text();
...
void DataManager::requestRoads(double lat,double lon,double rad)
{
    ...
    qDebug() << minLat << maxLat << minLon << maxLon;

    //preparing query
    query.prepare("SELECT id_way,centerLat,centerLon,id_node,oneway,roundabout,maxspeed,type,latitude,longitude FROM roads WHERE ((:minLat<=centerLat) and (centerLat<=:maxLat) and (:minLon<=centerLon) and (centerLon<=:maxLon))");
